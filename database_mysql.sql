...
DELIMITER / / CREATE
OR REPLACE PROCEDURE sp_read_semestres() BEGIN
SELECT
    id_semestre,
    nombre,
...
OR REPLACE FUNCTION fn_create_semestre(
    ...
    p_fecha_fin DATE,
    p_estado CHAR(1)
) RETURNS VARCHAR(255) BEGIN DECLARE semestre_existe INT;
...
DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN GET DIAGNOSTICS CONDITION 1 error_code = MYSQL_ERRNO,
...
SET
    error_message = 'Semestre ya existe';
...
IF error_code <> 99999 THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = error_message;
...
DELIMITER / / CREATE
OR REPLACE FUNCTION fn_delete_semestre(p_id INT) RETURNS VARCHAR(100) BEGIN DECLARE mensaje VARCHAR(100);
...
DELIMITER / / CREATE
OR REPLACE FUNCTION fn_update_estado(p_id_semestre INT, p_estado CHAR(1)) RETURNS VARCHAR(255) BEGIN DECLARE error_message VARCHAR(255);
