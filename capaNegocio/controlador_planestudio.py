...
#        cursor.execute("select * from fn_read_planEstudio()")
...
#aqui agrege
def agregar_plan_estudio(nombre, estado):
    conexion = obtener_conexion()
    msg = []
    with conexion.cursor() as cursor:
        cursor.execute(
            "SELECT fn_agregar_plan_estudio(%s, %s)",
            (nombre, estado),
        )
        msg = cursor.fetchone()
    conexion.commit()
...
def actualizar_plan_estudio(id, nombre, estado):
    conexion = obtener_conexion()
    msg = []
    with conexion.cursor() as cursor:
        cursor.execute(
            "SELECT fn_editar_plan_estudio(%s, %s, %s)",
            (id, nombre, estado),
        )
        msg = cursor.fetchone()
    conexion.commit()
